...
//		var a = $scope.datas();
	...
	$scope.edit = function() {
		$http.post("/role/update", $scope.editRole).success(function(response) {
	...
	$scope.shopRoleId = {};
	$scope.roleAuthority = function(shopRoleId) {
		$http.post('/role/roleAuthority').success(function(response) {
			$scope.shopRoleId = shopRoleId;
	...
	$scope.accredit = function() {
		var shopRoleId = $scope.shopRoleId;
		var authorityIds = $scope.selectRoleIds;
		$http.post('/role/accredit/' + shopRoleId, $scope.selectRoleIds).success(
				function(response) {
					if (response.status == 200) {
						alert(response.msg);
						$scope.reloadList();
...
//				beforeClick : beforeClick,
   ...
   $(".sub").on("click",function(){
	   var $this = $(this);
	  var name = $(this).closest(".top").find(".name").val();
	  var nameId = $(this).closest(".top").find(".idd").html()/1;
	  var textareas = $(this).closest(".top").find(".textareas").val();
	  //var idd =ID;
	  var option ={
			  roleName :name ,
			  parentId:nameId,
