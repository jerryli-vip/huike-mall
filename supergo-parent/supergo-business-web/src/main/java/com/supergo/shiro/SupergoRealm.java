...
public class SupergoRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		//获取当前登录用户
		TbShopUser user = (TbShopUser)principalCollection.getPrimaryPrincipal();
		//调用用户服务,根据用户名查询它所拥有的权限集合list<>
		List<TbAuthority> list = service.queryAuthorityCode(user.getShopUserName());
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken arg0) throws AuthenticationException {
		...
		// 调用service,查询用户是否存在
		TbShopUser user = service.selectByShopUserName(username);
		if (user == null) {
			return null;
		}
		...
		//第二个参数：凭证，其实指的就是密码
		//第三个参数：就是当前的realm的名称
		AuthenticationInfo info = new SimpleAuthenticationInfo(user, user.getShopUserPassword(), this.getClass().getSimpleName());
		return info;
	}
