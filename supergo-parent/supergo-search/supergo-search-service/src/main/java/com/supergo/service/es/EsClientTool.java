package com.supergo.service.es;
...
import com.supergo.service.util.Constants.ES;
import com.supergo.service.util.Constants.SEPARATOR;
...
public class EsClientTool {
	 ...
	 * 批处理组件插入
	 */
	public static void addDocsBukProcessor(String index, String type, List<GoodsEs> list) {
		BulkProcessor bulkProcessor = getBulkProcessor();

		for (GoodsEs goodsEs : list) {
			String json = JsonUtils.objectToJson(goodsEs);
			bulkProcessor.add(new IndexRequest(index, type, goodsEs.getGoodsId().toString()).source(json));
		}
		// 刷新所有请求
		bulkProcessor.flush();
		// 关闭bulkProcessor
	...
	public static List<Map<String, Object>> responseToList(SearchResponse response) {
		...
		for (int i = 0; i < hits.getHits().length; i++) {
			...
			//插入高亮信息
			Map<String, HighlightField> higlight = hit.getHighlightFields();
			for (String key : higlight.keySet()) {
				map.put(key,higlight.get(key).getFragments()[0].toString());
			}
			list.add(map);
		}
		return list;
