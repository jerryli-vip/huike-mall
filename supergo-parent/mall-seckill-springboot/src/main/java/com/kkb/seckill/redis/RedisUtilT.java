...
public class RedisUtilT {
	...
	private static final Logger LOG = LoggerFactory.getLogger(RedisUtilT.class);
 ...
 public <T> boolean exists(KeyPrefix prefix, String key) {
	 ...
	 */
	public static long incr(KeyPrefix prefix, String key) {
	...
	public static long decr(KeyPrefix prefix, String key) {
		// LOG.debug(" incr key :{}", key);
		try {
			if (isClose() || isEmpty(key)) {
				return 0;
	...
	public static boolean set(KeyPrefix prefix, String key, Object obj) {
		// LOG.debug(" set key :{}, value:{}", key, obj);
		try {
			if (isClose() || isEmpty(key) || isEmpty(obj)) {
				return false;
			...
			if (seconds <= 0) {
				...
				redisTemplate.opsForValue().set(realKey, obj, seconds);
			}
		} catch (Exception e) {
			LOG.error(e.getMessage(), e);
		}
