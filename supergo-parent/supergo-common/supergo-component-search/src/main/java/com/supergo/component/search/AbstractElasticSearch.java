package com.supergo.component.search;
...
public abstract class AbstractElasticSearch<T> {
	...
	public BulkResponse addDocByBulk(List<T> list) {
		...
		for (T t : list) {
			...
			try {
				builder.add(
						client.prepareIndex(getIndex(), getType()).setSource(JSON.toJSONString(t), XContentType.JSON));
				// 每1000条刷新提交一次
				if (count % 1000 == 0) {
					builder.execute().actionGet();
	...
	public void addDoc(Object id, T bean) throws Exception {
		...
		client.prepareIndex(getIndex(), getType(), id.toString()).setSource(JSON.toJSONString(bean), XContentType.JSON)
				.get();
	}
	...
	public List<T> multiGet(Object[] ids) throws Exception {
		...
		for (MultiGetItemResponse response : multiGetResponse) {
			...
			if (gr != null && gr.isExists() && !StringUtils.isEmpty(gr.getSourceAsString())) {
	...
	public List<T> query(QueryBuilder queryBuilder, int from, int size, List<SortBuilder> sortBuilders/*, Page<T> page*/) {
		...
		SearchRequestBuilder searchRequestBuilder = client.prepareSearch(getIndex()).setTypes(getType())
	...
	/*	if (CollectionUtils.isNotEmpty(page.getExclude())) {
		...
		logger.info("es query hit total {} ,hits cost time [{} ms] ", response.getHits().getTotalHits(),
				endTime - startTime);
	 ...
	 * @return
	 */
	// public List<T> boolQuery(QueryBuilder must, QueryBuilder mustNot,
	// QueryBuilder should, QueryBuilder filter,
	// int from, int size) {
	 ...
	 * @return
	 */
	// public List<T> termsQuery(String fieldName, Object[] value, int from, int
	// size) {
	 ...
	 * @return
	 */
	// public List<T> multiMatchQuery(String[] fieldName, Object value, int from,
	// int size) {
